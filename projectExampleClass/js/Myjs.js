...
function WordCloud(trips){
	...
	const scale = d3.scaleLinear().domain(d3.extent(Object.values(rs),x=>x.value)).range([1,100]);
	...
	function draw(words) {
	  d3.select("#rightside").append("svg")
		...
		.selectAll("text")
		  .data(words)
		.enter().append("text")
		  .style("font-size", function(d) { return scale(d.value) + "px"; })
		  .style("fill", function(d) { return d3.schemeDark2[Math.floor(scale(d.value)/10)]; })
		  .style('opacity',.7)
